...
public class AltaFrecuentesPagoTarjetaOtrosBancosCommand extends CommandBase{
	
	public Response ejecucion(Session session)throws Exception{
		Response response = new Response();
		ClienteTO clienteTO = (ClienteTO)session.getAttribute(CLIENTE_TO);
		AltaFrecuentesPagoTarjetaOtrosBancosForm frecuentesPagoTarjetaOtrosBancosForm = (AltaFrecuentesPagoTarjetaOtrosBancosForm)getFormBean();
		try{
			...
			if(session.getAttribute(CONSULTA_FRECUENTES_PAGO_TARJETA_OTROS_BANCOS_RESPONSE) == null){				
			...
			frecuentesRequestTO.setTarjeta(frecuentesPagoTarjetaOtrosBancosForm.getNumero_tarjeta());
			frecuentesRequestTO.setAlias(frecuentesPagoTarjetaOtrosBancosForm.getAlias_beneficiario());
			
			
			if( frecuentesPagoTarjetaOtrosBancosForm.getOpcion_seguridad().equalsIgnoreCase( TAG_TOKEN ) ) {
				frecuentesRequestTO.setOptionDispositive( OPCION_TOKEN );
				frecuentesRequestTO.setTokencode( frecuentesPagoTarjetaOtrosBancosForm.getClave_seguridad().toString() );
			} else if (frecuentesPagoTarjetaOtrosBancosForm.getOpcion_seguridad().equalsIgnoreCase( TAG_HUELLA ) ) {
				frecuentesRequestTO.setOptionDispositive( OPCION_HUELLA );
				frecuentesRequestTO.setClave( frecuentesPagoTarjetaOtrosBancosForm.getHuella_seguridad().toString() );
			}

			FrecuentesResponseTO frecuentesResponseTO = resourceFacadeSL.setAgregaCuentasFrecuentesTarjetasOtrosBancos(frecuentesRequestTO);

			Iterator<Map> iterator = frecuentesResponseTO.getTarjetasFrecuentes().iterator();
