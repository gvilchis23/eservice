...
public class FrecuentesTiempoAireEliminarCommand extends CommandBase {
	...
	public Response validacion(Session session) throws PagoServiciosException, SessionExpiredException, ServiceLocatorException{
		...
		try{
			...
			if(index == -1){
				...
				errors.put("numero_telefonico","Este número de teléfono celular no existe en su registro.");
				returnErrorMap(response, errors);
			}else{
				frecuentesTiempoAireRequestTO.setIndex(""+index);
				session.addAttribute(INDICE_REFERENCIA_ELIMINAR, frecuentesTiempoAireRequestTO.getIndex());
				session.addAttribute(COMPANIA_CELULAR_REFERENCIA, frecuentesTiempoAireRequestTO.getCompania());
				resourceFacadeSL.setBorraReferenciasFrecuentesTiempoAireValidacion(frecuentesTiempoAireRequestTO);
			...
			session.deleteAttribute(INDICE_REFERENCIA_ELIMINAR);
			session.deleteAttribute(COMPANIA_CELULAR_REFERENCIA);
	...
	public Response ejecucion(Session session) throws PagoServiciosException, SessionExpiredException, ServiceLocatorException{
		...
		try{
			...
			String compania = (String)session.getAttribute(COMPANIA_CELULAR_REFERENCIA);
			String index = (String)session.getAttribute(INDICE_REFERENCIA_ELIMINAR);
			if(compania!=null || index != null){
				if(compania.equalsIgnoreCase(forma.getCompania())){
					
					frecuentesTiempoAireRequestTO.setCompania(forma.getCompania());					
					frecuentesTiempoAireRequestTO.setIndex(index);
					frecuentesTiempoAireResponseTO = resourceFacadeSL.setBorraReferenciasFrecuentesTiempoAireEjecucion(frecuentesTiempoAireRequestTO);
					...
					//error si index no es igual a la compania elegida en validacion
					errors = new HashMap<String, String>();
					errors.put("compania","Estimado usuario el tipo de compania celular que esta intentando de introducir es incorrecto favor de verificar.");
					returnErrorMap(response, errors);
				}
			}else{
				//error si index es nulo
				errors = new HashMap<String, String>();
				errors.put("compania","Estimado usuario el tipo de solicitud que esta intentando de ejecutar es incorrecto favor de verificar su petición.");
				returnErrorMap(response, errors);
			}			
