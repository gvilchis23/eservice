...
public class SincronizacionFirmaForm extends FormBean{
	...
	public MessageErrors validate(){
		...
		if(getComando().equalsIgnoreCase("VALIDACION") || getComando().equalsIgnoreCase("EJECUCION")){
			if(Validator.isEmptyData(numero_serie)){
				errores.add("numero_serie", SINCRONIZACION_FIRMA_NUMERO_SERIE);
			}
			else {
				if (!Validator.checkNumeric(numero_serie)) {
					errores.add("numero_serie", SINCRONIZACION_FIRMA_NUMERO_SERIE_DECIMAL);					
				}
			}
			if(clave_seguridad == null || Validator.isEmptyData(clave_seguridad.toString())){
				errores.add("clave_seguridad", SINCRONIZACION_FIRMA_CLAVE_SEGURIDAD, "Firma Azteca");
			}
		}
		return  errores;
	}
